...
if __name__ == '__main__':
    ...
    # Asking for the MALE or FEMALE voice, now with error handling.
    audio = "" # Initialize audio variable
    try:
        with mic as source:
            print("Listening for voice choice (Male/Female)...")
            recognizer.adjust_for_ambient_noise(source, duration=0.2)
            audio_data = recognizer.listen(source)
        
        # Recognize the speech
        audio = recognizer.recognize_google(audio_data).lower()

    except sr.UnknownValueError:
        print("Could not understand audio. Defaulting to Male voice.")
        audio = "male" # Default to male if not understood
    except sr.RequestError as e:
        print(f"Could not request results from Google Speech Recognition service; {e}")
        audio = "male" # Default to male on connection error
...
#nklfenvvhkverkhvfjnrwjfnrnwjnrkjvbnfuvbvrufnrwnfjrvjkbrefjrwbhbrbhbrbrjvbjrjrwnjfrbvjr